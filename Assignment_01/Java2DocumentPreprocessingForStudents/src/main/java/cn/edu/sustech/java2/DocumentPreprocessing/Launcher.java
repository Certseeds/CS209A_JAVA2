    ...
    public void parseConfigFile(String configFilePath) {
        ...
        try (BufferedReader yamlReader = new BufferedReader(
                new InputStreamReader(
                        new FileInputStream(configFilePath), StandardCharsets.UTF_8.name()))) {
        ...
        if (configMap.containsKey("CategorizeBy")) {
            categorizeCriteriaList = (
                    (List<String>) configMap.get("CategorizeBy")).stream()
        ...
        try {
            for (FileEntry entry : fileEntries) {
                ...
                if (!file_name.contains(".")) {
                    currentPath.append(prefix + file_name + postfix);
                } else {
                    String[] pair = file_name.split("\\.");
                    currentPath.append(prefix + pair[0] + postfix + "." + pair[1]);
                }
