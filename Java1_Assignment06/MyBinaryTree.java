...
public class MyBinaryTree<T> {
    ...
    public void addTreeNodes(String directions, T[] values) {
        ...
        for (int i = 0; i < bytes.length; i++) {
            ...
            if (bytes[i] == '0') {
                if (origin.left_node == null) {
                    this.size++;
                    origin.left_node = next;
                } else {
                    origin.left_node.value = next.value;
                }
                origin = origin.left_node;
            } else {
                if (origin.right_node == null) {
                    this.size++;
                    origin.right_node = next;
                } else {
                    origin.right_node.value = next.value;
                }
                origin = origin.right_node;
            }
        }
    }
